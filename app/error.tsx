...
export default function ErrorPage({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  useEffect(() => {
    const code = error.digest?.replace(/[^a-zA-Z0-9_.:-]+/g, "_").slice(0, 128);
    reportTechnicalEvent({ kind: "page_error", area: "page", code: code || "page_boundary" });
  }, [error.digest]);
  useEffect(() => { if (ready) trackEvent("exception", { fatal: true, error_area: "page" }); }, [ready]);
