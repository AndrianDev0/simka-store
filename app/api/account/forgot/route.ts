...
function response(request: Request) {
  return NextResponse.redirect(new URL("/account/forgot?sent=1", request.url), 303);
}
...
export async function POST(request: Request) {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254) return response(request);
  ...
  if (account) {
    ...
    const resetUrl = new URL(`/account/reset?token=${encodeURIComponent(token)}`, request.url).toString();
    try {
