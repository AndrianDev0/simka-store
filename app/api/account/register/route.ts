...
export async function POST(request: Request) {
  try {
    ...
    if (name.length < 2 || name.length > 100 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254 || contact.length > 100 || !validatePassword(password) || password !== confirmation) {
      return redirect(request, "/account/register?error=Проверьте%20данные%20и%20пароль%20(минимум%208%20символов)");
    }
    const db = getDb();
    if ((await db.select({ id: customerAccounts.id }).from(customerAccounts).where(eq(customerAccounts.email, email)).limit(1))[0]) {
      return redirect(request, "/account/login?error=Аккаунт%20с%20этим%20email%20уже%20существует");
    }
    ...
    return redirect(request, "/account/register?error=Не%20удалось%20создать%20аккаунт");
  }
}
