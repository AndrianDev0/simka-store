...
function Checkout({items,total,currency,onBack,onOrder}:{items:CartLine[];total:number;currency:string;onBack:()=>void;onOrder:(receipt:OrderReceipt)=>void}){
  ...
  return <form className="flex flex-1 flex-col overflow-y-auto p-6" onSubmit={submit}>
    ...
    <div className="space-y-4"><FormField name="customerName" label="Имя" placeholder="Как к вам обращаться" autoComplete="name"/><FormField name="customerEmail" label="Email для получения реквизитов" placeholder="mail@example.com" email autoComplete="email"/><p className="-mt-2 text-xs leading-5 text-muted-foreground">Менеджер отправит реквизиты для оплаты на этот email. <a href="/account/login?returnTo=/" className="font-bold text-[#1168e8] hover:underline">Войти в кабинет</a></p><FormField name="customerContact" label="Telegram или телефон" placeholder="@username" optional autoComplete="tel"/>
...
function FormField({name,label,placeholder,email=false,optional=false,autoComplete,maxLength}:{name:string;label:string;placeholder:string;email?:boolean;optional?:boolean;autoComplete:string;maxLength?:number}){return <label className="block text-sm font-semibold" htmlFor={name}>{label}{optional&&<span className="font-normal text-muted-foreground"> — необязательно</span>}<Input id={name} name={name} required={!optional} type={email?"email":"text"} autoComplete={autoComplete} maxLength={maxLength??(email?254:100)} className="mt-2 h-11 rounded-xl" placeholder={placeholder}/></label>}
