...
export default function Storefront({categories=[],products,customer}:{categories?:PublicCategory[];products:Product[];customer?:CheckoutCustomer}) {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <header className="sticky top-0 z-40 border-b border-border/80 bg-white/90 backdrop-blur-xl"><div className="mx-auto flex h-[72px] max-w-[1240px] items-center gap-8 px-4 sm:px-6">
      ...
      <div className="ml-auto hidden items-center gap-2 md:flex"><Button asChild variant="ghost" className="h-11 rounded-xl text-[#42526a]"><a href="/account">Кабинет</a></Button><Button asChild variant="ghost" className="h-11 rounded-xl text-[#42526a]"><a href="/contacts"><Headphones className="size-4" aria-hidden="true"/>Помощь</a></Button></div><ThemeToggle/>
    ...
    <section id="top" className="theme-hero relative overflow-hidden border-b border-[#dce7f4] bg-[#f7fbff]"><div className="relative mx-auto grid max-w-[1240px] items-center gap-8 px-4 py-12 sm:px-6 md:grid-cols-[1.1fr_.9fr] md:py-16"><div>
...
function Logo(){return <span className="theme-logo grid size-10 place-items-center rounded-[13px] text-white"><Wifi className="size-5"/></span>}
...
function HeroCards(){return <div className="relative mx-auto hidden h-[370px] w-full max-w-[430px] md:block" aria-hidden="true"><div className="absolute inset-x-12 top-16 h-56 rounded-full bg-[#4d75ff]/25 blur-3xl"/><span className="absolute right-1 top-1 z-30 inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/90 px-4 py-2 text-[11px] font-black uppercase tracking-[.08em] text-[#1168e8] shadow-[0_12px_34px_rgba(20,65,120,.14)] backdrop-blur"><Check className="size-3.5"/>Готово к поездке</span><HeroPlanCard className="left-3 top-11 w-[300px] rotate-[-2deg] bg-[linear-gradient(135deg,#102944,#194b78)]" code="TR" country="Турция" data="20 ГБ" days="30 дней" operator="Turkcell"/><HeroPlanCard className="bottom-1 right-0 w-[320px] rotate-[1.5deg] bg-[linear-gradient(135deg,#1168e8,#6747ee_58%,#22aeea)]" code="TH" country="Таиланд" data="∞ ГБ" days="15 дней" operator="AIS"/></div>}
function HeroPlanCard({className,code,country,data,days,operator}:{className:string;code:string;country:string;data:string;days:string;operator:string}){return <div className={`${className} absolute h-[160px] overflow-hidden rounded-[26px] border border-white/20 p-5 text-white shadow-[0_24px_60px_rgba(7,21,43,.28)]`}><span className="absolute -right-8 -top-16 size-40 rounded-full bg-white/10 blur-2xl"/><div className="relative"><div className="flex items-center justify-between"><span className="grid size-9 place-items-center rounded-xl border border-white/10 bg-white/10"><Wifi className="size-4.5"/></span><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-black tracking-[.14em]">{code}</span></div><div className="mt-4 flex items-end justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.12em] text-white/60">{country} · eSIM</p><p className="mt-1 text-2xl font-black tracking-tight">{data}</p></div><div className="text-right text-xs text-white/70"><p>{days}</p><p className="mt-1 font-bold text-white">{operator}</p></div></div></div></div>}

function Cart({open,onOpenChange,items,count,total,currency,mixedCurrencies,checkout,ordered,customer,onChange,onCheckout,onBeginCheckout,onOrder}:{open:boolean;onOpenChange:(open:boolean)=>void;items:CartLine[];count:number;total:number;currency:string;mixedCurrencies:boolean;checkout:boolean;ordered:OrderReceipt|null;customer?:CheckoutCustomer;onChange:(key:string,n:number)=>void;onCheckout:(v:boolean)=>void;onBeginCheckout:()=>void;onOrder:(receipt:OrderReceipt)=>void}){return <Sheet open={open} onOpenChange={onOpenChange}><SheetTrigger asChild><Button className="relative h-11 rounded-xl bg-[#10213a] px-4 text-white hover:bg-[#1b3456]"><ShoppingBag className="size-[18px]" aria-hidden="true"/><span className="hidden sm:inline">Корзина</span>{count>0&&<span aria-live="polite" className="absolute -right-2 -top-2 grid size-5 place-items-center rounded-full bg-[#b7f34a] text-[11px] font-black text-[#10213a]">{count}</span>}</Button></SheetTrigger><SheetContent className="w-full border-l-[#dce5f0] bg-white p-0 sm:max-w-[470px]"><SheetHeader className="border-b border-border px-6 py-5"><SheetTitle className="text-xl">Ваша корзина</SheetTitle><SheetDescription>{count?`${count} ${productWord(count)}`:"Пока здесь пусто"}</SheetDescription></SheetHeader>{ordered?<OrderDone receipt={ordered}/>:checkout?<Checkout items={items} total={total} currency={currency} customer={customer} onBack={()=>onCheckout(false)} onOrder={onOrder}/>:items.length?<CartList items={items} total={total} currency={currency} mixedCurrencies={mixedCurrencies} onChange={onChange} onCheckout={()=>{onBeginCheckout();onCheckout(true)}}/>:<div className="flex flex-1 flex-col items-center justify-center px-8 text-center"><span className="mb-4 grid size-16 place-items-center rounded-full bg-[#edf5ff] text-[#1168e8]"><ShoppingBag className="size-7"/></span><h3 className="text-xl font-bold text-[#10213a]">Выберите подходящую SIM</h3><p className="mt-2 text-sm text-muted-foreground">Добавленные тарифы появятся здесь.</p></div>}</SheetContent></Sheet>}
...
function ProductCard({product,count,onAdd}:{product:Product;count:number;onAdd:()=>void}){
  ...
  return <article className="theme-card group overflow-hidden rounded-[22px] border border-[#dce5ee] bg-white transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(23,58,96,.12)]">
