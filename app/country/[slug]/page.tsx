...
import { getCatalogCountries, getCatalogCountryBySlug, getCatalogProducts } from "@/lib/catalog-repository";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const [country, products] = await Promise.all([getCatalogCountryBySlug(decoded), getCatalogProducts()]);
  ...
  return pageMetadata({
    ...
    noindex: country.noindex || items.length === 0,
  });
}
...
export default async function CountryPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const decoded = decodeSlug(slug);
  const [countries, products, categories] = await Promise.all([getCatalogCountries(), getCatalogProducts(), getPublicCategories()]);
  let country = countries.find((item) => item.slug.toLowerCase() === decoded.toLowerCase());
  if (!country) {
    ...
    const legacyProduct = products.find((product) => slugify(product.country) === decoded.toLowerCase());
    country = legacyProduct ? countries.find((item) => item.id === legacyProduct.countryId) : undefined;
    if (country) permanentRedirect(`/country/${encodeURIComponent(country.slug)}`);
    notFound();
  }
