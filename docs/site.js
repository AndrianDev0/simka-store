(() => {
  ...
  function renderHome() {
    ...
    app.innerHTML = `<div class="hero-section"><section class="hero shell"><div><div class="eyebrow">◎ SIM и eSIM для путешествий</div><h1>eSIM и SIM<br><span>для путешествий</span></h1><p>Выберите страну, получите eSIM с инструкцией после оплаты или закажите физическую SIM с доставкой.</p><form id="home-search" class="hero-search"><input name="q" type="search" aria-label="Поиск тарифа" placeholder="Куда планируете поездку?"><button class="button gradient-button" type="submit">Найти тариф →</button></form><div class="hero-checks"><span>Цена и условия до заказа</span><span>Инструкция по активации</span><span>Поддержка на русском</span></div></div><div class="hero-art" aria-hidden="true"><div class="sim-card"><div class="card-copy"><small>eSIM · Турция</small><strong>Интернет в поездке</strong><small>Подключение по QR-коду</small></div><div><span class="chip"></span><span class="wifi">◉</span></div></div><div class="sim-card"><div class="card-copy"><small>eSIM · Таиланд</small><strong>На связи везде</strong><small>Сразу после оплаты</small></div><div><span class="chip"></span><span class="wifi">◉</span></div></div></div></section></div>${state.categories.length ? `<section class="section-tint"><div class="section shell"><div class="section-head"><div><div class="eyebrow">Категории</div><h2>Подборки SIMKA</h2></div><a class="section-link" href="#/categories">Все категории →</a></div><div class="grid">${state.categories.slice(0, 3).map(categoryTile).join("")}</div></div></section>` : ""}<section class="section shell"><div class="section-head"><div><div class="eyebrow">Каталог тарифов</div><h2>Выберите тариф для поездки</h2><p class="section-intro">Сравните интернет, срок действия и оператора. Актуальные цены и наличие проверяются при заказе.</p></div><a class="section-link" href="#/catalog">Все тарифы →</a></div>${productGrid(featured)}</section><section class="section-tint"><div class="section shell"><div class="section-head"><div><div class="eyebrow">Простой процесс</div><h2>Как это работает</h2></div></div><div class="grid">${infoCard("01 · Выберите тариф", "Укажите страну и сравните предложения по интернету, сроку и цене.")}${infoCard("02 · Оформите заказ", "Оставьте email. Менеджер пришлёт реквизиты для оплаты.")}${infoCard("03 · Получите SIM", "После подтверждения оплаты eSIM придёт на email, физическая SIM отправится доставкой.")}</div></div></section><section class="section shell"><div class="section-head"><div><div class="eyebrow">Помощь</div><h2>Частые вопросы</h2></div><a class="section-link" href="#/faq">Все вопросы →</a></div><div class="info-grid">${pages.faq.cards.slice(0, 4).map(([title, text]) => infoCard(title, text)).join("")}</div></section>`;
  }
  ...
  function renderProduct(slug) {
    ...
    const current = offer(product);
    app.innerHTML = `${heading(`${product.country} · ${product.operator}`, product.name, product.shortDescription || "Тариф для поездки")}${body(`<div class="breadcrumb"><a href="#/">Главная</a> / <a href="#/catalog">Каталог</a> / ${escape(product.country)} / ${escape(product.name)}</div><div class="detail-layout"><div class="detail-visual tone-${productTheme(product)}"><span class="type">${escape(product.type)}</span>${flag(product.country, product.countryId, product.flag)}<h2>${escape(current.data)}</h2><p>${escape(product.country)} · ${escape(current.days)} дней</p></div><div class="detail-panel"><div class="price">${money(current.price, current.currency)}</div><p class="muted">${isAvailable(product) ? "В наличии" : "Нет в наличии"} · Артикул: ${escape(current.variant?.sku || product.sku)}</p><ul class="detail-list"><li>Страна<br><strong>${escape(product.country)}</strong></li><li>Оператор<br><strong>${escape(product.operator)}</strong></li><li>Интернет<br><strong>${escape(current.data)}</strong></li><li>Срок<br><strong>${escape(current.days)} дней</strong></li>${product.calls ? `<li>Звонки<br><strong>${escape(product.calls)}</strong></li>` : ""}${product.compatibility ? `<li>Совместимость<br><strong>${escape(product.compatibility)}</strong></li>` : ""}</ul><button class="button gradient-button" data-add="${product.id}" ${isAvailable(product) ? "" : "disabled"}>${isAvailable(product) ? "Добавить в корзину +" : "Нет в наличии"}</button><p class="muted">Цена и наличие повторно проверяются при создании заказа.</p></div></div><section class="detail-extra"><h2>Описание тарифа</h2><p>${escape(product.fullDescription || product.shortDescription || "Уточните подробные условия у менеджера до оплаты.")}</p></section><section class="detail-extra"><h2>Получение и активация</h2><p>После подтверждения оплаты eSIM и инструкция отправляются на email. Для физической SIM менеджер согласует доставку.</p></section>`)}`;
  }
