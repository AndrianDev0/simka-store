...
Аналитика включается только после согласия посетителя. Передаются обезличенные просмотры, поиск и фильтры, действия с товарами и корзиной, начало оформления, способ оплаты, создание заказа (`generate_lead`), подтверждённая продажа (`purchase`), отмена/возврат, авторизация, ошибки и Web Vitals. `purchase` отправляется только когда клиент открывает заказ, которому менеджер уже присвоил статус оплаты. Имя, email, телефон, адрес и платёжные данные в аналитику не отправляются.
