...
Роли Telegram-админки:

- `owner` — все разделы, журнал и резервные копии;
- `admin` — полное операционное управление без резервных копий;
- `analyst` — аналитика, CSV, просмотр промокодов и партнёров, статус и технические ошибки;
- `marketing` — аналитика, CSV-отчёты, ввод рекламных расходов для CAC и управление промокодами и партнёрами;
- `sales` — заказы, их обработка, клиенты в режиме чтения, общая и партнёрская аналитика;
- `support` — только просмотр заказов и необходимых данных клиента.

Бот скрывает недоступные разделы и повторно проверяет права перед каждой командой, callback и ответом на force-reply. ID, которого нет в `TELEGRAM_ADMIN_IDS`, не получает доступа даже при наличии роли в `TELEGRAM_ADMIN_ROLES`.

Для отправки писем через Resend:

- подтвердите принадлежащий магазину домен или отдельный почтовый поддомен в Resend (SPF и DKIM);
- `RESEND_API_KEY` — серверный API-ключ с правом отправки;
- `EMAIL_FROM` — отправитель на подтверждённом домене, например `SIMKA <orders@notify.example.com>`;
- `EMAIL_REPLY_TO` — необязательный адрес поддержки, на который клиент может ответить.
